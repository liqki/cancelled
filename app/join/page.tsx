...
export default function Join({ searchParams }: { searchParams: { error: string; roomId: string } }) {
          ...
          <button
            ...
            onClick={() => {
              if (!inputRef.current?.value) {
                router.push("/load?error=Name is required");
                return;
              }
              router.push(`/${searchParams.roomId}/?name=${inputRef.current.value}`);
            }}
            type="submit"
          >
            Join Room
